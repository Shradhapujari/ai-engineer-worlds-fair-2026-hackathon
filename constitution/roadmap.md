...
- ☑ Demo script per §8 (`demo/run_demo.py`): work → break vendor live → heal (patch→scan→sandbox→hot-swap) → cache-hit kicker (no model call, hit_count++) → audit/kill-switch/breaker line. Offline fallback patch keeps demo path alive; `OMNIFORGE_DEMO_LIVE=1` uses real Gemini.
- ☑ **Verified LIVE end-to-end with real Gemini 3.5 Flash**: heal 14s → cache-hit 0.37s (no model call, hit_count=1) → audit log; agent source auto-restored. Both `-m live` tests green (30s).
...
- 2026-06-28 — **Live bring-up: full loop verified against real Gemini.** Fixed 4 real bugs surfaced only live: (1) `diagnose` now asks the model for the COMPLETE fixed file and computes the unified diff itself via difflib — LLM-emitted diffs were unusable (wrong hunk counts, missing/relative headers, zero context); (2) Vertex call uses `response_schema` (constrained decoding) so multiline code is always valid JSON; (3) `patcher` normalizes diff header paths + falls back to GNU `patch --fuzz`; (4) `sandbox` runs in *overlay* mode (project copy + patched file at its real path, copy first on PYTHONPATH) so package imports hit the PATCHED code, not the installed original. Demo `run_demo.py` heals live in ~14s then cache-hits in 0.37s. 77 unit + 2 live tests green. Contract change: model returns `fixed_source` not `unified_diff`.
